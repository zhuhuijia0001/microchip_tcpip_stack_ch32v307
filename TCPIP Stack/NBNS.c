...
void NBNSTask(void)
{
	...
	switch(smNBNS)
	{
		...
		case NBNS_LISTEN:
        	...
        	memcpypgm2ram(FormattedNetBIOSName, (ROM void*)AppConfig.NetBIOSName, 16);
            FormatNetBIOSName(FormattedNetBIOSName);
			...
			while(NBNSHeader.Questions.Val--)
			{
				...
				if(Type.Val == 0x0020u && Class.Val == 0x0001u && memcmp((void*)NameString, (void*)FormattedNetBIOSName, sizeof(FormattedNetBIOSName)) == 0)
				{
					...
					NBNSPutName(FormattedNetBIOSName);
